...
################################################################################################
var_imu_f = 0.10 #initial 0.1
var_imu_w = 0.25 #initial 0.25
var_gnss  = 0.01 #initial 0.01
var_lidar = 1.00 #initial 1.00 p1:1 p2:100 p3:100
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    Rk = sensor_var * np.eye(3)
    # [3, 3]
    Kk = p_cov_check @ h_jac.T @ inv(h_jac @ p_cov_check @ h_jac.T + Rk)

    # 3.2 Compute error state
    error_state = Kk @ (y_k - p_check)
    ...
    q_hat = Quaternion(axis_angle=error_state[6:]).quat_mult(q_check)
